...
class LanzaroteRentSpider(scrapy.Spider):
	name = 'lanzarote_rent'
	...
	def parse(self, response):
		...
		self.driver.get("https://www.kyero.com/en/lanzarote-apartments-long-let-1l55569g1?sort=popularity_desc")
		sleep(2)
		...
		for page in range(pages_count):
			...
			self.driver = webdriver.Chrome(str(Path(Path.cwd(), "chromedriver.exe")), chrome_options=options)
			self.driver.set_window_size(randrange(1100, 1200), randrange(800, 900))
			self.driver.get(f"https://www.kyero.com/en/lanzarote-apartments-long-let-1l55569g1?min_beds=2&page={page}&sort=popularity_desc")
			sleep(1)
			body = self.driver.find_element_by_css_selector('body')
			sleep(1)

...
dataset = pd.read_csv("mitula.csv", encoding="utf-8")



dataset = dataset[~dataset["address"].str.contains("colligco")]
dataset["baths"] = dataset["baths"].str.split(" ").str[0].astype(int)
dataset["beds"] = dataset["beds"].str.split(" ").str[0].astype(int)


dataset = dataset.drop(["link", "popularity", "property_type", "size"], axis=1)
...
dataset.to_csv("ballincollig_mitula.csv", index=False)

...
CSV_CLEANED = list(Path.cwd().glob('*.csv'))

CSV_FILES = [str(Path(filename).stem) for filename in CSV]
CSV_FILES_CLEANED = [str(Path(filename).stem) for filename in CSV_FILES]
...
def merge_and_date(CSV_FILES):
    ...
    for file in CSV_FILES:
        try:
            ...
            dataset["baths"] = dataset["baths"].str.split(" ").str[0]
            dataset["beds"] = dataset["beds"].str.split(" ").str[0]
            dataset["popularity"] = dataset["popularity"].str.replace(",", "")
            dataset["price"] = dataset["price"].str.replace(",", "").str.replace("\€", "")
            dataset["size"] = dataset["size"].str.replace(" m²", "")
            datasets.append(dataset)
            dataset.to_csv(f"{file}_cleaned.csv",  encoding='utf-8-sig', index=False) 
        except:
            pass
...
dataset = merge_and_date(CSV_FILES)  
...
dataset.to_csv("ballincollig_offers_2021.csv", index=False)
